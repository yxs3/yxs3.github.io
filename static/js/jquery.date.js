...
(function (factory) {
    ...
    createDate = {
        ...
        resetActive:function(){
             var d = new Date();
            if(opt.limitTime){
                d.setDate(d.getDate()+(opt.limitTime -1));
            }
    ...
    $('#d-confirm').bind('click',function(){
        ...
        }else if(opt.limitTime == '3'){
            var d = new Date(),
                error = !isEnglish ? '晚上10点后，至少选择后天':'At least the day after tomorrow';
            //当前日期+2
            var day = String(d.getFullYear())+'-'+String(d.getMonth() + 1)+'-'+String(d.getDate()+2);
            var d1 = new Date(str.replace(/\-/g, "\/")); 
            var d2 = new Date(day.replace(/\-/g, "\/"));
            if(d1 < d2){
                alert(error);
